...
const renderCurrentWeather = location => {
    ...
    const settings = {
        url: `https://cors-anywhere.herokuapp.com/http://api.openweathermap.org/data/2.5/weather?q=${location}&units=imperial&appid=59c9f9d794fad5805a3ff6749acde141
        `,
        method: 'GET'
    };
    ...
    $.ajax(settings).done(async response => {
        ...
        let uvJSON = await $.ajax({url: `https://cors-anywhere.herokuapp.com/http://api.openweathermap.org/data/2.5/uvi?lat=${response.coord.lat}&lon=${response.coord.lon}&appid=59c9f9d794fad5805a3ff6749acde141`, method: 'GET'});
        console.log(uvJSON);
        ...
        //Cloud cover icon
        $('.icon').html(`<img src='https://cors-anywhere.herokuapp.com/http://openweathermap.org/img/wn/${weatherIcon}.png'/>`);
...
const renderFiveDay = (lat, lon) => {
    ...
    const settings = {
        url: `https://cors-anywhere.herokuapp.com/http://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&units=imperial&exclude=hourly&appid=59c9f9d794fad5805a3ff6749acde141`,
        method: 'GET'
    };
    ...
    $.ajax(settings).done(response => {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            let $dayDiv = $(`<div class='day'><p>${date}</p><img src='https://cors-anywhere.herokuapp.com/http://openweathermap.org/img/wn/${weatherIcon}.png'/><p>${cloud}</p><p>Max: ${maxTemp} &#176;F</p><p>Min: ${minTemp} &#176;F</p><p>Humidity: ${humidity} %</div>`);
